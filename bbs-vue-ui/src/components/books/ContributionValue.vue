<template>
  <div id="contribution-value" :style="$store.state.collapsed ? 'padding: 10px 10px 0 10px;' : 'padding: 50px 50px 20px 50px;'">
    <div class="markdown-body">
      <h1>贡献值、等级与权限说明</h1>
      <br/>
      <h2>贡献值</h2>
      <p>在社区论坛里我们会使用 <strong>贡献值</strong> 来计算一个用户的累计贡献值，累计贡献值的目的是为了更好地回馈优质的用户，同时也借由他们生产的优质内容帮助到更多的用户们。您可以在您的个人主页上看到自己的贡献值。</p>
      <h3>贡献值的价值</h3>
      <ol>
        <li>贡献值会影响到你发的文章的排序，贡献值高且活跃的用户所生产的内容会被推荐给更多的用户</li>
        <li>贡献值会影响你是否被推荐，越来越多的用户通过关注来消费内容，因而被推荐并获得关注可以有效地提高你的影响力</li>
        <li>贡献值会有不同的权限，随着贡献值的不断提高，越来越多的功能和权限会解锁，让你在社区论坛里畅行无阻</li>
      </ol>
      <h3>贡献值如何计算</h3>
      <p>贡献值的目的就是为了计算一名用户的累计贡献，而社区论坛的核心目的是去分享和学习有价值的内容，因而他与您分享的内容本身的价值直接相关：</p>
      <p style="color: red;">贡献值 = 文章的阅读数 / 10 + 获得的点赞数</p>
      <code class="hljs language-!" lang="!">
        1、贡献值只计算用户在社区论坛内的文章
        <br/>
        2、贡献值每天凌晨<span style="color: red;">1点</span>更新计算一次
        <br/>
        3、如果作者删除了自己的文章，相应的贡献值会被扣除，已经获得的权限也将被撤销
      </code>
      <br/>
      <h2>等级与权限</h2>
      <table>
        ...
        <tr>
          ...
          <td>成为社区论坛共建者<span style="color: red;">(这就是你的项目)</span></td>
        </tr>
        </tbody>
      </table>
      <h3>刷贡献值的惩罚</h3>
      <p>一经发现某一用户通过机器等手段刷点赞、阅读等数据，恶意提高自己的贡献值，社区论坛作者在核实后会给予惩罚，<span style="color: red;">贡献值归 0，撤销所有权限及身份。</span></p>
    </div>
  </div>
</template>
